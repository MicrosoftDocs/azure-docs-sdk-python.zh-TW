...
服務連線會針對整個迭代器維持開啟狀態。 如果您發現自己僅部分逐一查看訊息資料流，則應該在 `with` 陳述式中執行接收端，確保連線已關閉：
...
0.50.0 版引進了重大變更。 原始 HTTP 型 API 仍適用於 v0.50.0，不過現在存在於新的命名空間之下：`azure.servicebus.control_client`。
...
新服務匯流排命名空間不支援 ACS。 我們建議[將應用程式遷移至 SAS 驗證](https://docs.microsoft.com/azure/service-bus-messaging/service-bus-migrate-acs-sas)。 若要在舊版服務匯流排命名空間內使用 ACS 驗證，請使用下列程式碼建立 ServiceBusService：
